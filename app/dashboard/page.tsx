    ...
    <div className="flex h-screen w-full bg-background-dark text-white overflow-hidden">
      ...
      >
        <div className="p-8 flex items-center gap-4">
          ...
          >
            <span className="material-symbols-outlined text-white text-2xl font-black">hub</span>
          </motion.div>
        ...
        <nav className="flex-1 overflow-y-auto px-6 py-4 flex flex-col gap-2 scrollbar-hide">
          ...
          <div className="mt-6">
            ...
            <div className="space-y-2">
              {[
                { name: 'Linear Structures', href: '/ide/linear', icon: 'reorder', color: 'text-primary' },
      ...
      >
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-3 pl-2 group cursor-pointer">
                <div className="text-right hidden sm:block">
                  <p className="text-xs font-bold text-white leading-none">Vikram Purohit</p>
                  <p className="text-[10px] text-text-secondary font-mono mt-1 uppercase tracking-tighter">Senior Researcher</p>
                </div>
                <motion.div 
                  whileHover={{ scale: 1.05 }}
                  className="w-10 h-10 rounded-xl bg-gradient-to-br from-primary to-secondary p-[1px] group-hover:shadow-[0_0_15px_rgba(127,19,236,0.3)] transition-all"
                >
                  <div className="w-full h-full bg-surface-darker rounded-xl flex items-center justify-center overflow-hidden">
                    <span className="text-xs font-black tracking-tighter">VP</span>
                  </div>
                </motion.div>
              </div>
            </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { label: 'Algorithms Mastered', value: '12', trend: '+3', icon: 'psychology', color: 'text-primary' },
              { label: 'Total Lab Time', value: '48', unit: 'h', trend: 'Top 15%', icon: 'timer', color: 'text-secondary' },
              { label: 'Custom Scripts', value: '05', trend: 'Verified', icon: 'terminal', color: 'text-accent-mint' },
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              >
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-10">
                  <div className="space-y-6">
                    <div className="flex items-center gap-3">
                      <span className="px-3 py-1 bg-primary text-[10px] font-black text-white uppercase tracking-[0.2em] rounded-lg shadow-lg shadow-primary/20">Active Simulation</span>
                      <div className="flex items-center gap-2">
                        <span className="w-1.5 h-1.5 rounded-full bg-accent-mint animate-ping"></span>
                        <span className="text-[10px] font-mono text-text-secondary uppercase tracking-widest">#SIM-2026-X9</span>
                      </div>
                    </div>
                    <h3 className="text-5xl md:text-6xl font-black text-white tracking-tighter leading-[0.9] uppercase italic">
                      Quantum Search <br/>
                      <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary via-secondary to-accent-mint">Grover's Algorithm</span>
                    </h3>
                    <p className="text-text-secondary max-w-lg text-base leading-relaxed font-medium opacity-80">
                      Exploring amplitude amplification and phase inversion in unstructured databases. Current focus: Multi-qubit interference patterns and probability mapping.
                    </p>
                    <div className="flex items-center gap-6 pt-4">
                      <Link href="/ide/quantum">
                        <motion.button 
                      ...
                      <button className="flex items-center gap-3 text-xs font-black text-white/50 hover:text-primary transition-all uppercase tracking-widest group">
                        <span className="material-symbols-outlined text-[20px] group-hover:rotate-12 transition-transform">menu_book</span>
                        Module Docs
                      </button>
                    </div>
                  </div>
                  <div className="w-56 h-56 relative shrink-0 group-hover:scale-105 transition-transform duration-700">
                    <svg className="w-full h-full -rotate-90 drop-shadow-[0_0_30px_rgba(127,19,236,0.3)]" viewBox="0 0 100 100">
                      ...
                      <motion.circle 
                        ...
                        animate={{ strokeDashoffset: 282.7 * (1 - 0.65) }}
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      <span className="text-4xl font-black text-white tracking-tighter italic">65%</span>
            ...
            <div className="space-y-8">
              ...
              >
                <h3 className="text-sm font-black text-white uppercase tracking-widest mb-6 border-b border-border-dark pb-4">Activity Log</h3>
                <div className="space-y-6 relative">
                  <div className="absolute left-[19px] top-2 bottom-2 w-px bg-border-dark"></div>
                  {[
                    { title: 'Custom Stack Import', time: '2h ago', icon: 'terminal', color: 'text-primary' },
                    { title: 'RBT Module Complete', time: 'Yesterday', icon: 'check_circle', color: 'text-accent-mint' },
                    { title: 'Hosted Tree Lecture', time: '3 days ago', icon: 'school', color: 'text-lecture-primary' },
                  ].map((act, i) => (
                    <div key={i} className="flex gap-4 relative z-10">
                      <div className={`w-10 h-10 rounded-full bg-surface-dark border border-border-dark flex items-center justify-center ${act.color} shrink-0 shadow-xl`}>
                      ...
                      <div className="flex flex-col justify-center">
                        ...
                        <p className="text-[10px] text-text-secondary font-mono mt-1 uppercase">{act.time}</p>
                      </div>
                    </div>
                  ))}
                </div>
